...
![Spotlightでターミナルを検索している画面](images/01-01_spotlight_terminal.png)
*▲ Command+スペースで検索窓を開き「ターミナル」と入力する*

![ターミナルが開いた状態](images/01-02_terminal_open.png)
*▲ ターミナルが開いた状態。カーソルが点滅していればOK*
...
![GitHubのCodeボタン](images/02-01_github_code_button.png)
*▲ 緑色の「Code」ボタンをクリック*

2. 「**Download ZIP**」をクリック

![Download ZIPをクリック](images/02-02_github_download_zip.png)
*▲ 「Download ZIP」をクリックするとファイルがダウンロードされる*

3. ダウンロードされたZIPファイルを見つける（通常は「ダウンロード」フォルダ）
4. ZIPファイルをダブルクリックして解凍する

![ZIPを解凍した状態](images/02-03_zip_extracted.png)
*▲ 解凍するとフォルダが展開される*
...
![ファイルをmovie-uploadフォルダに移動した状態](images/02-04_files_moved.png)
*▲ movie-uploadフォルダの中にupload.pyなどが入っていればOK*
...
![新しいプロジェクトを作成する画面](images/04-01_new_project.png)
*▲ 「新しいプロジェクト」をクリック*
...
![YouTube Data API v3の検索結果](images/04-02_youtube_api_search.png)
*▲ 「YouTube Data API v3」をクリック*

4. 「**有効にする**」ボタンをクリック

![有効にするボタン](images/04-03_enable_api.png)
*▲ 「有効にする」をクリック*
...
1. 左上のメニュー →「**APIとサービス**」→「**OAuth同意画面**」をクリック
2. 「**外部**」を選択して「**作成**」をクリック

![外部を選択する画面](images/05-01_oauth_external.png)
*▲ 「外部」を選んで「作成」をクリック*
...
![アプリ名とメールを入力する画面](images/05-02_oauth_app_info.png)
*▲ アプリ名とメールアドレスを入力する*
...
![スコープを追加した画面](images/05-03_oauth_scopes.png)
*▲ 2つのスコープが追加された状態*
...
![テストユーザーを追加する画面](images/05-04_test_users.png)
...
![デスクトップアプリを選択する画面](images/05-05_desktop_app.png)
*▲ 「デスクトップ アプリ」を選択する*

5. 名前は何でもOK（例：`movie-upload-client`）→「**作成**」をクリック
6. ポップアップが表示されたら「**JSONをダウンロード**」をクリック

![JSONをダウンロードするポップアップ](images/05-06_download_json.png)
*▲ 「JSONをダウンロード」をクリック*
...
![完成したスプレッドシートの1行目の画面](images/06-01_spreadsheet_header.png)
*▲ 1行目にこの通りヘッダーを入力した状態*

### スプレッドシートのURLを確認する

スプレッドシートのURLには「シートID」が含まれています。

```
https://docs.google.com/spreadsheets/d/【スプレッドシートID】/edit?gid=【シートID】
```

![スプレッドシートのURLを確認する画面](images/06-02_spreadsheet_url.png)
*▲ URLの「gid=」の後の数字がシートID。設定時に使用する*
...
![設定ウィザードが起動した画面](images/07-01_config_wizard_start.png)
*▲ 「スプレッドシートのURLを入力してください」と表示された状態*
...
![設定ウィザードが完了した画面](images/07-02_config_wizard_done.png)
*▲ 「config.json を保存しました」と表示されれば完了*
...
![Googleの警告画面](images/08-01_google_warning.png)
*▲ この画面が出ても問題ありません*

1. 「**詳細**」をクリック
2. 「**（アプリ名）に移動（安全ではないページ）**」をクリック

![詳細をクリックした後の画面](images/08-02_google_warning_detail.png)
*▲ 「詳細」を押すと「移動」のリンクが現れる*

3. 「**続行**」をクリック

認証が完了するとブラウザに「The authentication flow has completed.」と表示されます。

![認証完了の画面](images/08-03_auth_complete_browser.png)
*▲ ブラウザにこの表示が出たらその画面は閉じてOK*

すべてのチャンネルで認証が完了すると、ターミナルに `全認証完了` と表示されます。

![ターミナルで全認証完了と表示された画面](images/08-04_auth_complete_terminal.png)
*▲ ターミナルに「全認証完了」と出ればOK*
...
![Zoom MarketplaceのDevelopボタン](images/10-01_zoom_marketplace.png)
*▲ 右上の「Develop」→「Build App」をクリック*

4. 一覧の中から「**Server-to-Server OAuth**」を見つけて「**Create**」をクリック

![Server-to-Server OAuthを選ぶ画面](images/10-02_server_to_server.png)
...
![Basic Informationの入力画面](images/10-03_basic_info.png)
...
![App Credentialsの画面](images/10-04_app_credentials.png)
*▲ 3つの情報をコピーしておく（画像では値をぼかしています）*
...
![スコープを追加する画面](images/10-05_add_scopes.png)
*▲ 2つのスコープにチェックを入れた状態*
...
![Activateボタンの画面](images/10-06_activate_button.png)
*▲ 「Activate your app」をクリック*

3. ステータスが「**Activated**」になればOK

![Activatedになった画面](images/10-07_activated.png)
...
![zoom-setupコマンドの実行画面](images/10-08_zoom_setup_terminal.png)
*▲ Account ID・Client ID・Client Secretの入力を求められる*
...
![L列のドロップダウンで削除OKを選ぶ画面](images/D-01_delete_ok_dropdown.png)
...
![ゴミ箱移動が成功した画面](images/D-02_trash_success_terminal.png)
*▲ 「✓ ゴミ箱に移動しました」と表示されれば成功*

4. Zoomのクラウドレコーディング画面で「ゴミ箱」を確認すると、録画が移動されています。

![Zoomのゴミ箱に録画が移動した画面](images/D-03_zoom_trash.png)
